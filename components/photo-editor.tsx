...
import { Upload, ZoomIn, ZoomOut, Download, RefreshCw, Settings, ChevronDown, ChevronUp } from "lucide-react"
...
export function PhotoEditor() {
  ...
  const [templateHeight, setTemplateHeight] = useState(DEFAULT_TEMPLATE_HEIGHT_MM)
  ...
  useEffect(() => {
    const calculateDimensions = () => {
      ...
      const containerWidth = isMobile ? Math.min(window.innerWidth - 32, 300) : 300
  ...
  const drawTemplateGuides = (ctx: CanvasRenderingContext2D) => {
    ...
    drawOval(ctx, centerX, centerY, ovalWidth * (maxFaceHeightPx / minFaceHeightPx), maxFaceHeightPx)
    ...
    ctx.lineWidth = 2
    drawOval(ctx, centerX, centerY, ovalWidth * 1.1, (minFaceHeightPx + maxFaceHeightPx) / 2)
    ...
    // Face height labels
    ctx.fillText(`Min: ${minFaceHeight.toFixed(1)}${units}`, centerX, centerY - minFaceHeightPx / 2 - 5)
    ctx.fillText(`Max: ${maxFaceHeight.toFixed(1)}${units}`, centerX, centerY - maxFaceHeightPx / 2 - 5)
  }
  ...
  const updateHighResCanvas = () => {
    ...
    const highResWidth = Math.round(widthMm * MM_TO_INCH * PX_PER_INCH * RESOLUTION_MULTIPLIER)
    const highResHeight = Math.round(heightMm * MM_TO_INCH * PX_PER_INCH * RESOLUTION_MULTIPLIER)
  ...
  const drawOval = (ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number) => {
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            {image && (
              <p className="text-sm text-center text-gray-500 mt-2">Drag to position your face within the oval guide</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div
              ...
              style={{ width: dimensions.width, height: dimensions.height }}
            >
              <canvas ref={previewCanvasRef} width={dimensions.width} height={dimensions.height} />
              {!image && (
            ...
            {image && <p className="text-sm text-center text-gray-500 mt-2">This is how your final photo will look</p>}
          </CardContent>
        </Card>
      </div>
        ...
        {image && (
          <div className="grid gap-2">
            ...
            <div className="flex justify-center">
              <p className="text-xs text-gray-500 mr-2">Zoom: {(zoom * 100).toFixed(0)}%</p>
              <Button
                variant="outline"
                size="sm"
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <Button variant="ghost" size="sm" className="h-8 px-2" onClick={() => setShowSettings(!showSettings)}>
            ...
            {showSettings ? (
              <div className="grid gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center mt-1">
                      <Input
                        ...
                        step="0.1"
                        value={templateWidth}
                        onChange={(e) => handleDimensionChange(e.target.value, setTemplateWidth)}
                      />
                  ...
                  <div>
                    ...
                    <div className="flex items-center mt-1">
                      <Input
                        ...
                        onChange={(e) => handleDimensionChange(e.target.value, setTemplateHeight)}
                      />
                      <span className="ml-2">{units}</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center mt-1">
                      <Input
                        ...
                        step="0.1"
                        value={minFaceHeight}
                        onChange={(e) => handleDimensionChange(e.target.value, setMinFaceHeight)}
                      />
                      <span className="ml-2">{units}</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center mt-1">
                      <Input
                        ...
                        step="0.1"
                        value={maxFaceHeight}
                        onChange={(e) => handleDimensionChange(e.target.value, setMaxFaceHeight)}
                      />
                      <span className="ml-2">{units}</span>
                    </div>
                  </div>
                ...
                <Button variant="outline" size="sm" onClick={resetToDefaults} className="w-full mt-2">
                  Reset to Default Dimensions
                </Button>
              </div>
            ) : (
              <ul className="space-y-1 text-sm">
                ...
                <li>
                  <strong>Standard resolution:</strong> {physicalDimensions.widthPx}px × {physicalDimensions.heightPx}px
                </li>
                <li>
                  <strong>Download resolution:</strong> {physicalDimensions.highResWidthPx}px ×{" "}
                  {physicalDimensions.highResHeightPx}px ({RESOLUTION_MULTIPLIER}x)
                </li>
              </ul>
            )}
          </CardContent>
    ...
    <main className="min-h-screen p-4 md:p-8 bg-gray-50">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-center mb-8 text-gray-600 max-w-2xl mx-auto">
          Upload your portrait photo, position your face within the guidelines, and download a high-resolution ID
        </p>
        <PhotoEditor />
      </div>
    </main>
